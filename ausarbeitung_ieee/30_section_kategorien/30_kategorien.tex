...
\begin{figure}[t!]
	...
	\includegraphics[width=0.5\textwidth]{../images/strategies_fig_02a.png} \\
	\small Quelle: \citep[Abbildung~2]{henrard-2002}
\end{figure}
...
\begin{figure*}[!t]
	...
	\includegraphics[width=0.8\textwidth]{../images/strategies_fig_02b.png} \\
	\small Quelle: \citep[Abbildung~2]{henrard-2002}
\end{figure*}

Auf konzeptueller Ebene spielt die fachliche Betrachtung vorhandener Daten eine wichtige Rolle. Das Verfahren ist nicht zwangsweise trivial. Analysen und konzeptuelle Konvertierung ben"otigen dabei ein gewisses Verst"andnis der Semantik entsprechender Daten. Bezogen auf das genannte Szenario (Siehe \ref{ref:scenario}) erfolgt eine Neukonzeptionierung der zugrundeliegenden Daten. Auf Basis der an das neue System angepassten Gesch"aftsprozesse erfolgt auch eine semantische Modernisierung der Datenquellen.
\lb
...
Um den Aufwand f"ur "Anderungen innerhalb von Softwaresystemen gering zu halten, bietet sich die Implementation eines Wrappers oder Adapters an. Dieser stellt eine Schnittstelle zwischen dem urspr"unglichen, unver"anderten Anwendungssystem und der neu strukturierten Datenquelle dar. Der Wrapper wird dabei als zus"atzliche Schicht zwischen beiden Komponenten eingef"ugt. Er nimmt Aufrufe der urspr"unglichen Anwendung entgegen, "ubersetzt sie in Anfragen an die neue Datenquelle und konvertiert die Ergebnisse der Anfrage f"ur das urspr"ungliche System. Er fungiert als Vermittler zwischen beiden Systemen und erspart so den Aufwand umfangreicher "Anderungen f"ur Abfragen im Anwendungssystem. Anhand des Beispiels (Siehe \ref{ref:scenario}) l"asst sich dies erl"autern: Die Datenquellen des Systems werden neu konzipiert, das urspr"ungliche System greift "uber die selbe Schnittstelle wie zuvor nicht mehr auf die urspr"ungliche Quelle, sondern durch den Wrapper als Vermittler auf die neue Datenquelle zu.

\begin{figure}[h!]
	\centering
	\caption{Einsatz eines Wrappers auf Anwendungsebene}
	\label{pic:application_wrapper}
	\includegraphics[width=0.49\textwidth]{../images/large_scale_fig_01.png} \\
	\small Quelle: \citep[Abbildung~1]{henrard-2008}
\end{figure}

Abbildung \ref{pic:application_wrapper} zeigt das Schema zur Nutzung eines Wrappers. Die Abstraktion erfolgt dabei auf Basis von Prozeduren zum Aufruf der neuen Datenquelle durch den Wrapper. Vor der Einbindung des Wrappers m"ussen diese aus der Legacy-Anwendung extrahiert werden. Nur so kann der sp"atere Aufruf des Wrappers anstatt der urspr"unglichen Datenquelle erfolgen \cite{henrard-2008}. 
\lb
...
%Beispiel \ref{code:logicbefore} zeigt den urspr"unglichen Zugriff des Legacy-Systems. Die angepasste Logik ist in Beispiel \ref{code:logicafter} einzusehen. Als Unterschied zur reinen Anpassung der Statements sind die Auswirkungen auf andere Programmteile meist gr"o"ser \citep{henrard-2002}.
